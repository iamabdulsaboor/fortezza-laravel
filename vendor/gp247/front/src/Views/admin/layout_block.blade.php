                ...
                <div class="card-header with-border">
                    ...
                    <div class="card-tools">
                        <div class="btn-group float-right mr-5">
                            <a href="{{ gp247_route_admin('admin_layout_block.index') }}" class="btn  btn-flat btn-default" title="List"><i class="fa fa-list"></i><span class="hidden-xs"> {{ gp247_language_render('admin.back_list') }}</span></a>
                        </div>
                    </div>
                </div>
                <!-- /.card-header -->
                ...
                <form action="{{ $url_action }}" method="post" accept-charset="UTF-8" class="form-horizontal" id="form-main"  enctype="multipart/form-data">
                    ...
                    <div class="card-body">
                            <div class="form-group row  {{ $errors->has('name') ? ' text-red' : '' }}">
                                <label for="name" class="col-sm-2 col-form-label">{{ gp247_language_render('admin.layout_block.name') }}</label>
                                <div class="col-sm-8">
                            ...
                            <div class="form-group row {{ $errors->has('position') ? ' text-red' : '' }}">
                                <label for="position" class="col-sm-2 col-form-label">{{ gp247_language_render('admin.layout_block.select_position') }}</label>
                                <div class="col-sm-8">
                                    <select class="form-control position select2" style="width: 100%;" name="position" >
                                        <option value=""></option>
                                        @foreach ($layoutPosition as $k => $v)
                                            <option value="{{ $k }}" {{ (old('position',$layout['position']??'') ==$k) ? 'selected':'' }}>{{ gp247_language_render($v) }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <span style="cursor: pointer;" onclick="imagedemo('https://static.gp247.net/page/block-template.jpg');"><i class="fa fa-question-circle" aria-hidden="true"></i></span>
                            </div>

                            <div class="form-group row {{ $errors->has('page') ? ' text-red' : '' }}">
                                <label for="page" class="col-sm-2 col-form-label">{{ gp247_language_render('admin.layout_block.select_page') }}</label>
                                <div class="col-sm-8">
                                    <select class="form-control page select2" multiple="multiple" style="width: 100%;" name="page[]" >
                                        <option value=""></option>
                                        @php
                                            $layoutPage = ['*'=> gp247_language_render('admin.position_all')] + $layoutPage;
                                            $arrPage = explode(',', $layout['page']??'');
                                        @endphp
                                        @foreach ($layoutPage as $k => $v)
                                            <option value="{{ $k }}" {{ in_array($k,old('page',$arrPage)) ? 'selected':'' }}>{{ gp247_language_render($v) }}</option>
                                        @endforeach
                                    </select>
                                        @if ($errors->has('page'))
                                            <span class="form-text">
                            ...
                            <div class="form-group row {{ $errors->has('type') ? ' text-red' : '' }}">
                                <label class="col-sm-2 col-form-label">{{ gp247_language_render('admin.layout_block.type') }}</label>
                                <div class="col-sm-8">
                            @if ($layout)
                            ...
                            <div class="form-group row {{ $errors->has('text') ? ' text-red' : '' }}">
                                <label for="text" class="col-sm-2 col-form-label">{{ gp247_language_render('admin.layout_block.text') }}</label>
                                <div class="col-sm-8">
                                    @php
...
@if (gp247_store_check_multi_partner_installed() ||  gp247_store_check_multi_store_installed())
                            ...
                            <div class="form-group row {{ $errors->has('store_id') ? ' text-red' : '' }}">
                                <label for="store_id" class="col-sm-2 col-form-label">{{ gp247_language_render('admin.select_store') }}</label>
                                <div class="col-sm-8">
    ...
    @endif
                            ...
                            <div class="form-group row  {{ $errors->has('sort') ? ' text-red' : '' }}">
                                <label for="sort" class="col-sm-2 col-form-label">{{ gp247_language_render('admin.layout_block.sort') }}</label>
                                <div class="col-sm-8">
                                    <div class="input-group">
                                        <div class="input-group-prepend">
                                        <span class="input-group-text"><i class="fas fa-pencil-alt"></i></span>
                            ...
                            <div class="form-group row ">
                                <label for="status" class="col-sm-2 col-form-label">{{ gp247_language_render('admin.layout_block.status') }}</label>
                                <div class="col-sm-8">
...
$(function () {
    ...
    if(type =='html'){
       obj.before('<textarea name="text" class="form-control text" rows="5" placeholder="Layout text"></textarea><span class="form-text"><i class="fa fa-info-circle"></i> {{ gp247_language_render('admin.layout_block.helper_html') }}.</span>');
       obj.remove();
    }else if(type =='view'){
        ...
        $.ajax({
            method: 'get',
            url: '{{ gp247_route_admin('admin_layout_block.listblock_view') }}?store_id='+storeId,
        ...
        $.ajax({
            method: 'get',
            url: '{{ gp247_route_admin('admin_layout_block.listblock_page') }}?store_id='+storeId,
            success: function (data) {
                obj.before(data);
                obj.remove();
                $('#loading').hide();
    ...
    $('[name="store_id"]').change(function(){
        ...
        if (type_checked == 'view') {
            url_type = '{{ gp247_route_admin('admin_layout_block.listblock_view') }}?store_id='+storeId;
        }
        if (type_checked == 'page') {
            url_type = '{{ gp247_route_admin('admin_layout_block.listblock_page') }}?store_id='+storeId;
        }
        $('#loading').show();
        $.ajax({
            method: 'get',
